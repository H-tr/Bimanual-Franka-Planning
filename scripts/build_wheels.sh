...
#
# Builds bimanual_franka-vamp version-specific wheels for Python 3.10, 3.11, 3.12.
...
docker run --rm \
  ...
  bash -c '
    ...
    # ------------------------------------------------------------------
    # Step 3: Build bimanual_franka-vamp version-specific wheels
    # ------------------------------------------------------------------
    PYTHON_VERSIONS="cp312-cp312 cp311-cp311 cp310-cp310"

    for PYVER in $PYTHON_VERSIONS; do
      echo ""
      echo "==> Building bimanual_franka-vamp wheel for $PYVER..."
    ...
    # ------------------------------------------------------------------
    echo ""
    echo "==> Repairing bimanual_franka-vamp wheels..."
    for whl in /tmp/raw-wheels/bimanual_franka_vamp-*.whl; do
      echo ""
      echo "    --- $(basename $whl) ---"
    ...
    # ------------------------------------------------------------------
    # Step 5: Build bimanual_franka-planning wheel
    # (No KDL in this container → pytracik is skipped → pure Python wheel)
    # ------------------------------------------------------------------
    echo ""
    echo "==> Building bimanual_franka-planning wheel..."
