...
Same 17-DOF cell as :mod:`bimanual_franka` (left arm + relative
3-DOF planar base + right arm) with the standard parallel-jaw
fingers replaced by a pair of soft-rubber fingers
(``soft_gripper_finger.stl``).  Arm kinematics and the
relative-base layout are unchanged — only the per-arm gripper
geometry differs, which is enough to require a dedicated cricket
FK header (``ext/ompl_vamp/robot/bimanual_fr3_soft.hh``).

Joint order in the 17-DOF state vector (matches the cricket-
...
# Atomic joint groups — identical to bimanual_franka.
JOINT_GROUPS = {
...
# Same FR3 datasheet limits as bimanual_franka.
