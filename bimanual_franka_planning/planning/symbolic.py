...
class SymbolicContext:
    ...
    The context hides the planar-root encoding (the 17-DOF state vector
    uses ``[x, y, theta, j0..j20]``; pinocchio uses
    ``[x, y, cos(theta), sin(theta), j0..j20]``) and the mapping from
    the active subspace back to the full 17-DOF state via ``base_config``.
    """
    ...
    def __init__(
        ...
        full_names = list(bimanual_fr3_robot_config.joint_names)
        if subgroup == "bimanual_fr3":
