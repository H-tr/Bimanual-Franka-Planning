...
:class:`bimanual_franka_planning.planning.symbolic.SymbolicContext`.  The
...
:class:`bimanual_franka_planning.planning.constraints.Constraint`: same
...
def _cache_root() -> Path:
    ...
    to ``~/.cache/bimanual_franka_planning/costs``.
    """
    ...
    return (base / "bimanual_franka_planning" / "costs").resolve()
...
class Cost:
    ...
    vector.  Construction triggers (on cold cache):
        ...
        4. caching under ``~/.cache/bimanual_franka_planning/costs/<sha>/``
    ...
    def __post_init__(self) -> None:
        ...
        if not so_path.exists():
            sys.stderr.write(f"[bimanual_franka] compiling cost {sha[:8]}... ")
            sys.stderr.flush()
