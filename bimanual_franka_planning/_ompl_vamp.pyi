...
type checkers can resolve ``import bimanual_franka_planning._ompl_vamp``.
"""
...
class OmplVampPlanner:
    ...
    * ``OmplVampPlanner()`` — full body, 17 DOF (3 base + 14 joints).
    * ``OmplVampPlanner(active_indices, frozen_config)`` — subgroup
      ...
      in the 17-DOF state on every state and motion validity query.
    """

    @overload
    def __init__(self) -> None:
        """Create a full-body planner (17 DOF)."""
        ...
    @overload
    def __init__(
        self,
        ...
        Args:
            active_indices: Positions in the full 17-DOF state that this
                planner will plan over, in DOF order.
            frozen_config: 17-DOF stance to inject for every joint *not*
                in ``active_indices``.
        """
        ...
    def add_pointcloud(
    ...
    def validate(self, config: Sequence[float]) -> bool:
        ...
        planners expand it to a full 17-DOF state with the stored
    ...
    def validate_batch(
        ...
        Subgroup planners expand each to a full 17-DOF state with
    ...
    def set_full_body(self) -> None:
        """Switch back to full-body planning (17 DOF).
