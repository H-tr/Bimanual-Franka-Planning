...
def _isolated_constraint_cache(tmp_path_factory):
    ...
    The default cache lives under ``~/.cache/bimanual_franka_planning/`` and
    pollutes the user's machine; CI runners get a clean dir.
    """
    cache = tmp_path_factory.mktemp("constraint_cache")
    old = os.environ.get("BIMANUAL_FRANKA_CONSTRAINT_CACHE_DIR")
    os.environ["BIMANUAL_FRANKA_CONSTRAINT_CACHE_DIR"] = str(cache)
    yield
    if old is None:
        os.environ.pop("BIMANUAL_FRANKA_CONSTRAINT_CACHE_DIR", None)
    else:
        os.environ["BIMANUAL_FRANKA_CONSTRAINT_CACHE_DIR"] = old
...
def test_constraint_rejects_non_sx_q_sym():
    from bimanual_franka_planning.planning import Constraint
