...
def test_factory_resolves_left_and_right():
    pytest.importorskip("pytracik")
    from bimanual_franka_planning.kinematics import create_ik_solver

    left = create_ik_solver("left_arm")
    right = create_ik_solver("right_arm")
    assert left.num_joints == 7
    assert right.num_joints == 7
    assert left.ee_frame == "fr3_left_link8"
    assert right.ee_frame == "fr3_right_link8"
...
_LEFT_ARM_JOINT_NAMES = [f"fr3_left_joint{i}" for i in range(1, 8)]
...
def left_arm_pin_context():
    ...
    from bimanual_franka_planning.kinematics import create_pinocchio_context
...
def test_pinocchio_fk_matches_trac_ik(left_arm_pin_context, trac_left_arm):
    ...
    from bimanual_franka_planning.kinematics import compute_forward_kinematics
...
def test_pinocchio_jacobian_shape(left_arm_pin_context):
    from bimanual_franka_planning.kinematics import compute_jacobian
...
def collision_ctx():
    ...
    from bimanual_franka_planning.bimanual_franka import CHAIN_CONFIGS
    from bimanual_franka_planning.kinematics import build_collision_model
...
def test_add_pointcloud_obstacles_validates_shape(collision_ctx):
    from bimanual_franka_planning.kinematics import add_pointcloud_obstacles
...
def pink_left_arm():
    ...
    pytest.importorskip("pinocchio")
    from bimanual_franka_planning.kinematics import create_ik_solver
    from bimanual_franka_planning.types import PinkIKConfig
...
def test_pink_solve_returns_plain_ik_result(pink_left_arm):
    ...
    from bimanual_franka_planning.types import IKResult
